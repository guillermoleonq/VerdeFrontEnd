...
export default {
    ...
    methods: {
        loadData: function () {
            axios.get('http://localhost:9000/api/marcas').then(result => {
                this.listData = result.data
            })
        },
        findByid: function (id) {
            // metodo para consutlar por el ig del boton impreso en la vista
            axios.get('http://localhost:9000/api/marcas/' + id).then(result => {
            ...
            if (bandera == true) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete('http://localhost:9000/api/marcas/' + id).then(() => {
                            Swal.fire({
                                icon: 'success',
        ...
        dataAdd: function () {
            ...
            axios.post('http://localhost:9000/api/marcas', data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
        ...
        dataUpdate: function () {
            ...
            axios.put('http://localhost:9000/api/marcas/' + this.id, data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
                        title: "'El registro se guardó de forma correcta.'",
